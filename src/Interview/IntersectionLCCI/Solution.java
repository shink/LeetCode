 ...
 * @project: LeetCode
 * @date: 2020/4/12
 * @desp: 面试题16.03——交点, https://leetcode-cn.com/problems/intersection-lcci/
 */
...
public class Solution {
        ...
        if ((y4 - y3) * (x2 - x1) == (y2 - y1) * (x4 - x3)) {
            ...
            if ((y2 - y1) * (x3 - x1) == (y3 - y1) * (x2 - x1)) {
                ...
                if (isInside(x1, y1, x2, y2, x4, y4)) {
                    updateRes(res, (double) x4, (double) y4);
                }
                // 判断 (x1, y1) 是否在「线段」(x3, y3)~(x4, y4) 上
                if (isInside(x3, y3, x4, y4, x1, y1)) {
                    updateRes(res, (double) x1, (double) y1);
                }
                // 判断 (x2, y2) 是否在「线段」(x3, y3)~(x4, y4) 上
                if (isInside(x3, y3, x4, y4, x2, y2)) {
                    updateRes(res, (double) x2, (double) y2);
                }
            }
        } else {
            // 联立方程得到 t1 和 t2 的值
    ...
    private void updateRes(double[] res, double x, double y) {
